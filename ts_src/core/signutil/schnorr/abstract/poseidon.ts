...
import { IField, FpPow, validateField } from './modular';
...
export function validateOpts(opts: PoseidonOpts) {
    ...
    if (
        opts.reversePartialPowIdx !== undefined &&
        typeof opts.reversePartialPowIdx !== 'boolean'
    )
        throw new Error(
            `Poseidon: invalid param reversePartialPowIdx=${opts.reversePartialPowIdx}`,
        );
    ...
    if (
        !Array.isArray(opts.roundConstants) ||
        opts.roundConstants.length !== rounds
    )
