...
class Decimal {
    ...
    toString() {
        const factor = new bn_js_1.default(10).pow(
            new bn_js_1.default(this.data.fractionalDigits),
        );
    ...
    minus(b) {
        if (this.fractionalDigits !== b.fractionalDigits)
            throw new Error('Fractional digits do not match');
        const difference = this.data.atomics.sub(
            new bn_js_1.default(b.atomics),
        );
        if (difference.ltn(0))
     ...
     */
    multiply(b) {
        const product = this.data.atomics.mul(
            new bn_js_1.default(b.toString()),
        );
