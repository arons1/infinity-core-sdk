...
function createApi(secp256k1) {
    return {
        ...
        pointAddScalar: (p, tweak, compress) =>
            secp256k1.publicKeyTweakAdd(p, tweak, compress),
        pointFromScalar: (d, compress) =>
            secp256k1.publicKeyCreate(d, compress),
        privateAdd: (d, tweak) =>
...
function createTinySecp256k1Interface(secp256k1) {
    return {
        ...
        pointCompress: (p, compressed) =>
            Uint8Array.from(
                secp256k1.publicKeyConvert(
                    p,
                    compressed != null ? compressed : true,
                ),
            ),
        isPrivate: d => secp256k1.privateKeyVerify(d),
        pointFromScalar: (d, compressed) => {
            const r = secp256k1.publicKeyCreate(
                d,
                compressed != null ? compressed : true,
            );
            if (r == null) {
        ...
        sign: (h, d, _e) => Uint8Array.from(secp256k1.ecdsaSign(h, d)),
        verify: (h, Q, signature, _strict) =>
