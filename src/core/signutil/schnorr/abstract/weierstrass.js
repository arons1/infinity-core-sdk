    ...
    class Point {
        ...
        static fromHex(hex) {
            const P = Point.fromAffine(
                fromBytes((0, utils_1.ensureBytes)('pointHex', hex)),
            );
            P.assertValidity();
            return P;
        }
...
function weierstrass(curveDef) {
    ...
    const {
        ...
        toBytes(_c, point, isCompressed) {
            ...
            if (isCompressed) {
                return cat(
                    Uint8Array.from([point.hasEvenY() ? 0x02 : 0x03]),
                    x,
                );
            } else {
    ...
    class Signature {
        ...
        recoverPublicKey(msgHash) {
            ...
            const h = bits2int_modN(
                (0, utils_1.ensureBytes)('msgHash', msgHash),
            ); // Truncate hash
    ...
    function prepSig(msgHash, privateKey, opts = defaultSigOpts) {
        ...
        if (ent != null) {
            ...
            seedArgs.push(
                (0, utils_1.ensureBytes)('extraEntropy', e, Fp.BYTES),
            ); // check for being of size BYTES
        }
