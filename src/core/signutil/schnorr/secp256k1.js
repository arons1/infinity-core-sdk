...
const Fp = (0, modular_1.Field)(secp256k1P, undefined, undefined, {
    sqrt: sqrtMod,
});
...
function challenge(...args) {
    return modN(
        (0, utils_2.bytesToNumberBE)(taggedHash('BIP0340/challenge', ...args)),
    );
}
/**
...
function schnorrSign(
    ...
    const a = (0, utils_2.ensureBytes)('auxRand', auxRand, 32); // Auxiliary random data a: a 32-byte array
    const t = numTo32b(
        d ^ (0, utils_2.bytesToNumberBE)(taggedHash('BIP0340/aux', a)),
    ); // Let t be the byte-wise xor of bytes(d) and hash/aux(a)
    const rand = taggedHash('BIP0340/nonce', t, px, m); // Let rand = hash/nonce(t || bytes(P) || m)
    const k_ = modN((0, utils_2.bytesToNumberBE)(rand)); // Let k' = int(rand) mod n
...
function schnorrVerify(signature, message, publicKey) {
    ...
    try {
        const P = lift_x((0, utils_2.bytesToNumberBE)(pub)); // P = lift_x(int(pk)); fail if that fails
        const r = (0, utils_2.bytesToNumberBE)(sig.subarray(0, 32)); // Let r = int(sig[0:32]); fail if r ≥ p.
        if (!fe(r)) return false;
        const s = (0, utils_2.bytesToNumberBE)(sig.subarray(32, 64)); // Let s = int(sig[32:64]); fail if s ≥ n.
...
(_a = htf.createHasher(
    ...
    (exports.hashToCurve = _a.hashToCurve),
    (exports.encodeToCurve = _a.encodeToCurve);
